...
describe('POST /v1/meme-replies', () => {
  ...
  it('reports an unavailable account store without masquerading as image generation', async () => { const brokenAuth = { ...auth, register: vi.fn(async () => { throw new Error('database connection refused'); }) } as unknown as AuthService; const response = await request(createApp({ auth: brokenAuth, store, generator })).post('/auth/register').send({}); expect(response.status).toBe(503); expect(response.body.code).toBe('AUTH_SERVICE_UNAVAILABLE'); expect(response.body.message).toBe('账户服务暂时不可用，请检查数据库连接后再试。'); });
